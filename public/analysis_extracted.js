        ...
        function loadProperty(index) {
            ...
            document.getElementById('prop-parcel').textContent = prop['Parcel #'];
            document.getElementById('prop-owner').textContent = prop['Name'];
            document.getElementById('prop-address').textContent = `${prop['Address']}, ${prop['City']}`;
            document.getElementById('prop-acres').textContent = prop['Acres'];
            document.getElementById('prop-amount').textContent = prop['Amount Due'];
            document.getElementById('prop-type').textContent = prop['Type'];
            document.getElementById('prop-county').textContent = prop['County'];
            
            // Legal Description with truncate and full tooltip
            const legalEl = document.getElementById('prop-legal');
            const legalFull = prop['Legal Desc'] || '-';
            const legalTruncated = legalFull.length > 50 ? legalFull.substring(0, 50) + '...' : legalFull;
            legalEl.textContent = legalTruncated;
            legalEl.title = legalFull;
            ...
            elevator.getElevationForLocations({
                ...
                if (status === 'OK' && results[0]) {
                    ...
                    if (elevationMeters < 3) {
                        riskLevel = '⚠️ ALTO RISCO';
                        riskColor = 'text-red-600';
                        riskIcon = '🚨';
                    } else if (elevationMeters < 10) {
                        riskLevel = '⚠️ RISCO MÉDIO';
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (details.buildings > 0) {
                        html += `<div class="mt-2 text-xs text-gray-600">🏠 ${details.buildings} construções próximas</div>`;
                    }
                    
                    if (analysis.roads > 0) {
                        html += `<div class="text-xs text-gray-600">🛣️ ${analysis.roads} estradas próximas</div>`;
                    }
                    ...
                    document.getElementById('land-use-content').innerHTML = '<p class="text-sm text-gray-600">Não foi possível analisar o uso do solo</p>';
                    document.getElementById('ndvi-content').innerHTML = '<p class="text-sm text-gray-600">Não foi possível calcular NDVI</p>';
                }
            } catch (error) {
                console.error('Land use error:', error);
                document.getElementById('land-use-content').innerHTML = '<p class="text-sm text-red-600">Erro ao carregar análise de uso do solo</p>';
            }
        }
        ...
        async function loadFemaFlood(location) {
            try {
                ...
                if (data.success && data.flood) {
                    ...
                    if (flood.insuranceRequired) {
                        html += `
                            <div class="bg-red-50 border border-red-200 rounded p-2 text-xs">
                                <strong class="text-red-700">⚠️ Seguro Obrigatório:</strong>
                                <span class="text-red-600">Esta propriedade requer seguro contra inundação</span>
                            </div>
                        `;
                    } else {
                        html += `
                            <div class="bg-green-50 border border-green-200 rounded p-2 text-xs">
                                <strong class="text-green-700">✅ Seguro Opcional:</strong>
                                <span class="text-green-600">Seguro contra inundação não é obrigatório</span>
                            </div>
                        `;
                    }
                    ...
                    document.getElementById('fema-flood-content').innerHTML = '<p class="text-sm text-gray-600">Não foi possível obter dados de risco de inundação</p>';
                }
            } catch (error) {
                console.error('FEMA flood error:', error);
                document.getElementById('fema-flood-content').innerHTML = '<p class="text-sm text-red-600">Erro ao carregar dados de inundação</p>';
            }
        }
        ...
        async function loadWaterProximity(location) {
            try {
                ...
                const waterTypes = [
                    { type: 'natural_feature', name: 'Corpo d\'Água', icon: '🌊' },
                    { type: 'park', name: 'Parque/Lago', icon: '🌳' }
                ];
                ...
                document.getElementById('water-proximity-content').innerHTML = 
                    '<p class="text-sm text-red-600">Erro ao carregar dados de proximidade de água</p>';
            }
        }
            ...
            if (closest.length === 0) {
                document.getElementById('water-proximity-content').innerHTML = `
                    <div class="flex items-center gap-2 text-sm">
                        ...
                        <span class="text-green-600 font-semibold">Nenhum corpo d'água próximo (raio 2km)</span>
                    </div>
                    <p class="text-xs text-gray-600 mt-1">Baixo risco de inundação por proximidade de água</p>
                `;
                return;
            }
            ...
            if (closestDistance < 100) {
                html += `
                    <div class="bg-red-50 border border-red-200 rounded p-2 text-xs mt-2">
                        <strong class="text-red-700">🚨 Alerta:</strong>
                        <span class="text-red-600">Propriedade muito próxima de água - alto risco de inundação</span>
                    </div>
                `;
            } else if (closestDistance < 500) {
                html += `
                    <div class="bg-orange-50 border border-orange-200 rounded p-2 text-xs mt-2">
                        <strong class="text-orange-700">⚠️ Atenção:</strong>
                        <span class="text-orange-600">Proximidade moderada de água - verificar histórico de inundações</span>
                    </div>
                `;
            }
        ...
        function calculateNDVI(landUsePercentages) {
            ...
            let html = `
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <div>
                        <div class="font-semibold text-sm text-${densityColor}-600">
                            Índice NDVI: ${ndvi.toFixed(2)}
                        </div>
                        <div class="text-xs text-gray-600">Densidade de Vegetação: ${density}</div>
                    </div>
                </div>
                
                <div class="mb-2">
                    <div class="text-xs text-gray-600 mb-1">Barra de Vegetação:</div>
                ...
                <div class="grid grid-cols-2 gap-2 text-xs">
                    <div class="bg-blue-50 border border-blue-200 rounded p-2">
                        <div class="text-blue-700 font-semibold">💰 Custo de Limpeza:</div>
                        <div class="text-blue-600">${clearingCost}</div>
                    </div>
                    <div class="bg-purple-50 border border-purple-200 rounded p-2">
                        <div class="text-purple-700 font-semibold">⏱️ Tempo Estimado:</div>
            ...
            if (ndvi >= 0.6) {
                html += `
                    <div class="bg-orange-50 border border-orange-200 rounded p-2 text-xs mt-2">
                        <strong class="text-orange-700">⚠️ Atenção:</strong>
                        <span class="text-orange-600">Vegetação densa - custo alto de limpeza. Considere negociar desconto no preço.</span>
                    </div>
                `;
            } else if (ndvi < 0.2) {
                html += `
                    <div class="bg-green-50 border border-green-200 rounded p-2 text-xs mt-2">
                        <strong class="text-green-700">✅ Vantagem:</strong>
                        <span class="text-green-600">Terreno limpo - pronto para construção com custo mínimo de preparação.</span>
                    </div>
                `;
            }
        ...
        function createGoogleEarthLink(location) {
            ...
            let html = `
                ...
                <div class="mt-2 text-xs text-gray-600">
                    <strong>🔍 O que você pode ver:</strong>
                    <ul class="list-disc list-inside mt-1 space-y-0.5">
                        <li>Imagens históricas desde 1984</li>
                        <li>Comparação lado a lado (2010 vs 2024)</li>
                        <li>Mudanças na vegetação e construções</li>
                        <li>Evolução da área ao longo dos anos</li>
                    </ul>
                </div>
            `;
            ...
            try {
                ...
                if (prop.lat && prop.lng) {
                    ...
                    if (!searchResponse.ok) {
                        throw new Error('Propriedade não encontrada no Zillow');
                    }
                    ...
                    if (!zpid) {
                        throw new Error('ZPID não encontrado');
                    }
                    ...
                    if (zillowPhotos.length === 0) {
                        document.getElementById('zillow-loading').textContent = 'Nenhuma foto disponível no Zillow';
                    } else {
        ...
        async function loadRealtorPhotos(prop) {
            ...
            try {
                ...
                if (realtorPhotos.length === 0) {
                    document.getElementById('realtor-loading').textContent = 'Nenhuma foto disponível no Realty Mole';
                } else {
