...
function gerarPSA() {
  ...
  // HTML do formulario (igual ao seu print)
...
function processPSAForm(formData) {
  ...
  const dueDay          = (formData.dueDay || '').trim(); // "15" ou "25"
  ...
  // Calcula PURCHASE_PRICE e BALANCE se quiser (aqui: Purchase = Down +  parcelas * valor)
  ...
  // Se ainda estiver vazio, garante string vazia
  ...
  const replacements = {
    ...
    '{{Email}}': buyer1Email || '',
    '{{Phone}}': '', // se quiser, depois colocamos o telefone em alguma coluna
  ...
  // Popup com dados para envio do PSA
  mostrarPSAPopup_({
...
function gerarContractForDeed() {
  ...
  // Se Effective Date estiver vazio, usa hoje
  if (!effectiveDate) {
  ...
  // Se First Payment Date estiver vazio, pergunta
  if (!firstPayDate) {
  ...
  // Monta replacements de acordo com o template do CFD
  const replacements = {
  ...
  // Popup com dados para envio do CFD
  mostrarCFDPopup_({
    buyerNames: buyerFullName,
    email: buyer1Email,
    propertyAddress: propertyAdr,
